...
const lowBarsData = ref([]);
onMounted(async () => {

  try {
    const querySnapshot = await getDocs(collection(db, 'low_bars'));
    lowBarsData.value = querySnapshot.docs.map(doc => doc.data());
  } catch (error) {
...
<template>
  <section id="Projects"
    ...
    <div class="w-72 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl animate-fade-down animate-duration-1000"
         v-for="product in lowBarsData" :key="product.id">
